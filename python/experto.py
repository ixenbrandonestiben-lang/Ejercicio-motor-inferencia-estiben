...
base_de_conocimiento = [
    ...
    {
        "id": "R08",
        "descripcion": "Puntero del mouse errático o lento",
        "condiciones": ["enciende", "mouse_lento", "puntero_salta"],
        "conclusion": "Limpiar el sensor óptico inferior con un paño seco y cambiar la superficie de uso por un mousepad opaco.",
        "confianza": 0.90
    },
    {
        "id": "R09",
        "descripcion": "Batería no retiene carga",
        "condiciones": ["enciende", "bateria_no_carga", "se_apaga_al_desconectar"],
        "conclusion": "Calibrar la batería mediante un ciclo completo de carga/descarga o realizar el reemplazo físico de la celda.",
        "confianza": 0.90
    },
    {
        "id": "R10",
        "descripcion": "Ausencia total de audio",
        "condiciones": ["enciende", "sin_sonido", "audio_no_reproduce"],
        "conclusion": "Verificar y cambiar el dispositivo de salida predeterminado en el panel de control de sonido de Windows.",
        "confianza": 0.85
    }
]
...
def inferir(base_conocimiento, hechos):
    ...
    regla = ordenar_diagnosticos(conflict_set)
    if not conflict_set:
        print('no se encontraron reglas aplicables')
        return
    
    ranking = ordenar_diagnosticos(conflict_set)
...
PREGUNTAS = {
    ...
    "mouse_lento":              '¿el mouse se pone lento o se queda sin movimiento',
    "puntero_salta":            '¿El mouse comienza a dar saltos o parpadeos?',
    "bateria_no_carga":         '¿La bateria no almacena la carga o no almacena la carga?',
    "se_apaga_al_desconectar":  '¿El equipo se apaga al desconectarlo despues de mantenerlo cargnando?',
    "audio_no_reproduce": 'Al reproducir algun video o musica se escucha bien.'
